...
<body>
    <div class="container">
        ...
        // 조회수 증가
        $sql_update_views = "UPDATE board_posts SET views = views + 1 WHERE id = $id";
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            ...
            if (in_array(strtolower($file_ext), $image_exts)) {
                // 이미지 파일일 경우 화면에 출력 및 다운로드 버튼 추가
                echo "<div class='image-container'>";
                echo "<img src='$file' alt='첨부 이미지'>";
                echo "</div>";
                echo "<p><a href='$file' download class='button'>이미지 다운로드</a></p>";
            } else {
                // 이미지가 아닐 경우 링크 제공
